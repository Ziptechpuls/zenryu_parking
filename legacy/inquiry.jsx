/* Static inquiry form — submits via mailto: and shows success state */
function InquiryForm({ defaultSubject, defaultPlan }) {
  const [subject, setSubject] = React.useState(defaultSubject || 'parking');
  const [plan, setPlan] = React.useState(defaultPlan || 'take');
  ...
  const handleSubmit = (e) => {
    ...
    const planLabel = subject === 'parking'
      ? ({ume:'梅プラン', take:'竹プラン', matsu:'松プラン'}[plan] || '未選択')
      : '';
    const body = [
...
function InquirySide({ phone, email, line }) {
  const c = window.ZENRYU_DATA.company;
  const usePhone = phone || c.phone;
  const useEmail = email || 'info@zenryurentacar.com';
  return (
...
function InquirySection({ id, defaultSubject, defaultPlan, eyebrow, title, lede }) {
  return (
...
Object.assign(window, { InquiryForm, InquirySide, InquirySection });
